      ...
      // B2. TODO - Listen for the 'load' event on the window object.
        window.addEventListener('load', (event) => {
          ...
          registration.then(value => {
            console.log("LOG: service worker registration successful");
          });
        });
      }
...
async function getRecipes() {
  ...
  const retrievalStatus = new Promise(async (resolve, reject) => {
    ...
    try {
      for (let i = 0; i < RECIPE_URLS.length; i++) {
        remoteRecipe = await fetch(RECIPE_URLS[i]);
        remoteJson = await remoteRecipe.json();
        recipeArr.push(remoteJson);
      }
